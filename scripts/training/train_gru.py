"""
Train the Bidirectional GRU model on historical OHLCV data.

Uses the TrainedGRU architecture from ml_model_loader (83-feature input,
2-layer BiGRU with hidden=134 per direction, classifier head).

Usage:
    python -m scripts.training.train_gru
    python -m scripts.training.train_gru --epochs 100 --lr 5e-4
"""
...
SAVE_PATH = os.path.join(PROJECT_ROOT, "models", "trained", "best_gru_model.pth")
...
def train_gru(
    ...
    logger.info(f"Training GRU on {device}")
    ...
    if len(X_train) == 0 or len(X_val) == 0:
        raise RuntimeError("Not enough data to generate sequences.")

    logger.info(f"Dataset sizes: train={len(X_train)}, val={len(X_val)}, test={len(X_test)}")

    model = TrainedGRU().to(device)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Bidirectional GRU")
