"""
Train the Bidirectional LSTM model on historical OHLCV data.

Uses the exact same TrainedBidirectionalLSTM architecture from ml_model_loader,
ensuring weight compatibility between training and inference.

Usage:
    python -m scripts.training.train_bidirectional_lstm
    python -m scripts.training.train_bidirectional_lstm --epochs 100 --lr 5e-4
"""
...
SAVE_PATH = os.path.join(PROJECT_ROOT, "models", "trained", "best_bidirectional_lstm_model.pth")
...
def train_bidirectional_lstm(
    ...
    model = TrainedBidirectionalLSTM().to(device)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Bidirectional LSTM")
    ...
    results = train_bidirectional_lstm(
        epochs=args.epochs,
