...
export default function VAEGauge() {
  ...
  const latest = data.length > 0 ? data[data.length - 1].vae_loss : 0;
  // Normalize VAE loss: typical range 0-0.5, anything > 0.3 is concerning
  const normalized = Math.min(latest / 0.5, 1);
          ...
          {data.length > 1 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data} margin={{ top: 2, right: 5, bottom: 2, left: 5 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#1a2235', border: '1px solid #243049', borderRadius: 8, fontSize: 11 }} labelFormatter={formatTimestamp} />
            ...
            <div className="h-full flex items-center justify-center text-gray-600 text-xs">No history</div>
          )}
        </div>
      </div>
      <p className="text-[10px] text-gray-600 mt-2">
        Latest reconstruction error: {latest.toFixed(6)} | Threshold: 0.3
      </p>
    </div>
  );
}
