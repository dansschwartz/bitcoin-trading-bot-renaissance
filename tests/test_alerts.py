...
def _run(coro):
    """Helper to run async code in tests (Python 3.10+)."""
...
class TestAlertManager(unittest.TestCase):
    ...
    def test_alert_logged(self):
        ...
        _run(self.am.send_alert("WARNING", "Test Alert", "Something happened"))
    ...
    def test_critical_alert_uses_critical_level(self):
        _run(self.am.send_alert("CRITICAL", "Kill Switch", "Emergency halt"))
        args = self.logger.log.call_args
        self.assertEqual(args[0][0], logging.CRITICAL)

    def test_warning_alert_uses_warning_level(self):
        _run(self.am.send_alert("WARNING", "Drift", "Position drift detected"))
        args = self.logger.log.call_args
        self.assertEqual(args[0][0], logging.WARNING)

    def test_alert_history_recorded(self):
        _run(self.am.send_alert("WARNING", "Test", "msg1"))
        _run(self.am.send_alert("CRITICAL", "Test2", "msg2"))
    ...
    def test_disabled_alerts_not_sent(self):
        ...
        _run(am.send_alert("CRITICAL", "Title", "msg"))
    ...
    def test_no_slack_webhook_graceful(self):
        ...
        _run(self.am.send_alert("WARNING", "Test", "no slack"))
        # Should not raise, and should log
        self.logger.log.assert_called_once()

    def test_get_recent_alerts_limit(self):
        for i in range(30):
            _run(self.am.send_alert("WARNING", f"Alert {i}", f"msg {i}"))
